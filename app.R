  ...
  div(
    ...
    div(
      ...
      p(
        class = "hero-text",
        HTML(paste0(
          as.character(i18n$t("Denne testen viser hvordan fravær av oppmerksomhets- og reguleringsvansker kan se ut.")),
          " ",
...
server <- function(input, output, session) {
  ...
  output$om_testen <- renderText({
      ...
      res <- switch (lg,
                     ...
                     fi = "Testiä on kokeiltu yli kolmensadan käyttäjän kanssa, ja se näyttää hyviä psykometrisiä ominaisuuksia. Tämä ei tarkoita, että testi voisi varmasti sanoa, ettei sinulla ole ADHD:ta, mutta se viittaa siihen, että mahdolliset vaikeudet johtuvat todennäköisesti jostakin muusta.</p><p>Tulokset viittaavat siihen, että testi tavoittaa ennen kaikkea yhden päämallin siinä, miten ihmiset säätelevät huomiota ja arkea. Samalla tällä mallilla näyttää olevan kaksi puolta: sisempi puoli, joka liittyy aloittamiseen, keskittymiseen ja ajatusten hallintaan, sekä ulompi puoli, joka liittyy järjestykseen, sovittuihin aikoihin ja käytännön luotettavuuteen. Testiä voi siis ymmärtää mittana siitä, kuinka paljon kitkaa ihminen kokee elämän eri osa-alueilla. Tällaiset vaikeudet voivat näkyä työssä tai opinnoissa, kotona, ihmissuhteissa, ajankäytössä ja käytännön askareissa. Matalammat pisteet viittaavat suurempaan samankaltaisuuteen ADHD:hen liittyvän säätelyvaikeusmallin kanssa, kun taas korkeammat pisteet viittaavat pienempään samankaltaisuuteen. Tarkemman katsauksen löydät <a href='/docs/fi/adhd_psychometric_note.pdf' target='_blank' rel='noopener'>tästä artikkelista</a>.</p><p>Käännökset on tehty koneellisesti ja tarkistettu mahdollisuuksien mukaan. Jos löydät virheitä, kerro ihmeessä.",
                     pl = "Test został wypróbowany na ponad trzystu użytkownikach i wykazuje dobre właściwości psychometryczne. Nie znaczy to, że może z całą pewnością stwierdzić, że nie masz ADHD, ale może wskazać, że istnieją dobre powody, by sądzić, że ewentualne trudności wynikają raczej z czegoś innego.</p><p>Wyniki sugerują, że test uchwytuje przede wszystkim jeden główny wzorzec tego, jak ludzie regulują uwagę i codzienne życie. Zarazem wzorzec ten zdaje się mieć dwie strony: bardziej wewnętrzną, związaną z zabieraniem się do pracy, utrzymywaniem skupienia i panowaniem nad myślami, oraz bardziej zewnętrzną, związaną z organizacją, terminami i praktyczną niezawodnością. Test można więc rozumieć jako miarę tego, ile tarcia dana osoba doświadcza w różnych obszarach życia. Takie trudności mogą być odczuwalne w pracy lub szkole, w domu, w relacjach, w gospodarowaniu czasem i w codziennych praktycznych sprawach. Niższe wyniki wskazują na większe podobieństwo do wzorca trudności z samoregulacją istotnego dla ADHD, a wyższe na mniejsze. Pełniejsze omówienie znajdziesz w <a href='/docs/pl/adhd_psychometric_note.pdf' target='_blank' rel='noopener'>artykule</a>.</p><p>Tłumaczenia są generowane maszynowo i w miarę możliwości sprawdzane. Jeśli znajdziesz błędy, daj znać.",
                     lt = "Testas išbandytas su daugiau nei trimis šimtais naudotojų ir pasižymi geromis psichometrinėmis savybėmis. Tai nereiškia, kad jis gali užtikrintai pasakyti, jog ADHD neturite, tačiau jis gali parodyti, kad yra svarių priežasčių manyti, jog jūsų patiriami sunkumai greičiausiai kyla dėl ko nors kito.</p><p>Rezultatai rodo, kad testas pirmiausia užfiksuoja vieną pagrindinį modelį, kaip žmonės reguliuoja dėmesį ir kasdienį gyvenimą. Kartu šis modelis turi dvi puses: labiau vidinę, susijusią su darbų pradėjimu, dėmesio išlaikymu ir minčių valdymu, ir labiau išorinę, susijusią su organizavimu, susitarimais ir praktiniu patikimumu. Todėl testą galima suprasti kaip matą, kiek trinties žmogus patiria įvairiose gyvenimo srityse. Tokie sunkumai gali pasireikšti darbe ar mokykloje, namuose, santykiuose, planuojant laiką ir kasdieniuose praktiniuose darbuose. Žemesni balai rodo didesnį panašumą į ADHD būdingą savireguliacijos sunkumų modelį, aukštesni mažesnį. Išsamesnį aptarimą rasite <a href='/docs/lt/adhd_psychometric_note.pdf' target='_blank' rel='noopener'>straipsnyje</a>.</p><p>Vertimai atlikti mašininiu būdu ir, kiek įmanoma, patikrinti. Jei rasite klaidų, praneškite.",
                     uk = "Тест було випробувано більш ніж на трьохстах користувачах, і він показує хороші психометричні властивості. Це не означає, що тест може з упевненістю сказати, що у вас немає ADHD, але він може вказати, що є вагомі підстави вважати, що будь-які труднощі, які ви відчуваєте, ймовірно пов’язані з чимось іншим.</p><p>Результати вказують на те, що тест насамперед вловлює один головний патерн у тому, як люди регулюють увагу й повсякденне життя. Водночас цей патерн має два боки: внутрішній, пов’язаний із тим, щоб почати справу, тримати фокус і керувати думками, та зовнішній, пов’язаний з організацією, домовленостями й практичною надійністю. Тож тест можна розуміти як міру того, скільки тертя людина відчуває в різних сферах життя. Такі труднощі можуть проявлятися на роботі або в навчанні, вдома, у стосунках, у використанні часу та в щоденних практичних завданнях. Нижчі бали вказують на більшу схожість із патерном труднощів саморегуляції, пов’язаним із ADHD, а вищі бали - на меншу схожість. Для докладнішого огляду дивіться <a href='/docs/uk/adhd_psychometric_note.pdf' target='_blank' rel='noopener'>статтю</a>.</p><p>Переклади зроблено машинно й по можливості перевірено. Якщо знайдете помилки, дайте знати.",                                        # fallback hvis språk mangler
